---
title: "Linear Regression with LiuLinReg"
...
The __LiuLinReg__ package is an alternative package to the lm package to make a linear regression. This document is not an introduction to linear regression. Furthermore it shall guide you to use the functions of the __LiuLinReg__ to perform a linear regression. The calculations are based on the `iris` dataset.
...
The `plot()` function will plot the residuals vs. the fitted values as well as the scale location using `ggplot2`. (You should load the package ggplot2 )
```{r}
library(ggplot2)
tmp$plot()
```
